...
A GQL query begins with the `GRAPH` keyword specifying the target graph, followed by a `MATCH` pattern and a `RETURN` clause.
- Nodes: `(var:Label)`
...
### 3. Output Formats: Tabular vs. Visualization
...
### 4. GQL Subquery Constraints (Crucial)
...
### 5. Relational Integration (`GRAPH_TABLE`)
...
4. **Mid-Path Sampling**: For deep or high fan-out queries, use `NEXT` chaining and intermediate window filters (e.g., `FILTER ROW_NUMBER() OVER (PARTITION BY ELEMENT_ID(source)) <= 5`) to sample paths mid-traversal.
